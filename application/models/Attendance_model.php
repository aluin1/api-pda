...
class Attendance_model extends CI_Model {	
	...
	protected 	$STA_Location = 'STA_Location';
...
public function set_attend_log(array $data)
	...
	public function vaksincheck($Id)
	{
		...
		public function vaksincheckkeluarga($Id)
	{
		$this->db->select("*");
		$this->db->from($this->wr_vaksin);
		$this->db->where('nopek', $Id);
...
/*	
	...
	{
		$this->db->select("FITUR_KEYVALUE"); 
		$this->db->where('FITUR', 'SUBJECT EMAIL');
		$this->db->where('FITUR_ACTIVE','1');
		$this->db->from($this->ms_fitur); 
		$query = $this->db->get();
		return $query;
	...
	{
		$this->db->select("FITUR_KEYVALUE"); 
		$this->db->where('FITUR', 'BODY EMAIL');
		$this->db->where('FITUR_ACTIVE','1');
		$this->db->from($this->ms_fitur); 
		$query = $this->db->get();
		return $query;
	...
	*/
	...
	public function sendEmail($url, $param)
	{ 
	...
	public function edit_vaksin($id, $vakke, $vaknama, $vakdate, $date)
	{
		
		 
		$this->db->set('VaccineNo', $vakke);
		$this->db->set('VaccineName', $vaknama);
		$this->db->set('VaccineDate', $vakdate);
		$this->db->set('LastUpdatedDate', $date);
		$this->db->where('Id', $id);
		$updating_data = $this->db->update($this->wr_vaksin);
		// return var_dump($updating_data);
		if ($updating_data > 0) {
			// return $data_update;
			return TRUE;
	...
	public function edit_isolasi($id, $nama, $keluhan, $date)
	{
		
		 
		$this->db->set('RelativesName', $nama);
		$this->db->set('Note', $keluhan); 
		$this->db->set('LastUpdatedDate', $date);
		$this->db->where('Id', $id);
		$updating_data = $this->db->update($this->sta_relative);
		// return var_dump($updating_data);
		if ($updating_data > 0) {
			// return $data_update;
			return TRUE;
	...
	public function edit_keluhan($id, $nama, $keluhan, $date)
	{
	...
	public function edit_phone($id, $nama, $phone, $date)
	{
		
		 
		$this->db->set('RelativesName', $nama);
		$this->db->set('PhoneNumber', $phone); 
		$this->db->set('LastUpdatedDate', $date);
		$this->db->where('Id', $id);
		$updating_data = $this->db->update($this->sta_relative);
		// return var_dump($updating_data);
	...
	public function edit_wr($id, $kegiatan, $hasil, $status)
	{
		
		 
		$this->db->set('WR_Kegiatan', $kegiatan);
		$this->db->set('WR_Hasil', $hasil); 
		$this->db->set('WR_Status', $status);
		$this->db->where('Id', $id);
		$updating_data = $this->db->update($this->wr_table);
		// return var_dump($updating_data);
		if ($updating_data > 0) {
			// return $data_update;
			return TRUE;
	...
	public function edit_vaksin_kel($id, $vakke,$nama, $vaknama, $vakdate, $date)
	{
		...
		public function datalokasicheck()
	{
		...
		$this->db->from($this->STA_Location);
		$this->db->order_by("LocId", 'asc'); 
		$query = $this->db->get();
		return $query;
	...
	public function deleteisolasi($id)
	{ 
		$this->db->where('Id', $id);
		$query = $this->db->delete($this->sta_relative);
		return $query;
	...
	public function getfiturkuis()
	{
		$this->db->select("*"); 
		$this->db->where('FITUR', 'KUIS');
		$this->db->where('FITUR_ACTIVE','1');
		$this->db->from($this->ms_fitur); 
		$query = $this->db->get();
		return $query;
	...
	public function getfitururl()
	{
		$this->db->select("*"); 
		$this->db->where('FITUR', 'URL_ATTENDANCE');
		$this->db->where('FITUR_ACTIVE','1');
		$this->db->from($this->ms_fitur); 
		$query = $this->db->get();
		return $query;
	...
	public function updateFit($nopek, $date, $statusFit, $noteFit, $noteFitlain)
	{
		$this->db->set('StatusFit', $statusFit);
		$this->db->set('StatusFitNote', $noteFit);
		$this->db->set('StatusFitNote_Lain', $noteFitlain);
		$this->db->where('nopek', $nopek);
		$this->db->where('date_attend', $date);
		$updating_data = $this->db->update($this->attend_table);
		if ($updating_data > 0) {
