        		...
        		if ($username != $token_valid->data->USERNAME) {
					...
					if ($condition != "come" && $condition != "return" && $condition != "cek_kondisi"  ) {
						...
						//Start Convert lat long to address
						  ...
						  if($resultGeoCode["status"] == "REQUEST_DENIED")
						  {
							  $data_geo["formatted_address"] = "";
							  $data_geo["country"] = "";
							  $data_geo["administrative_area_level_3"] = "";
							  $data_geo["administrative_area_level_2"] = "";
							  $data_geo["administrative_area_level_1"] = "";
						  }
						  else
						  {
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
					...
					if ($condition != "come" && $condition != "return" && $condition != "break" && $condition != "end_of_break" && $condition != "leave") {
						...
						//$pos_id_atasan = $token_valid->data->EMPLOYEE_NOPEK;
						//$pos_id_atasan = "742595";
								$get_atasan = $this->Attendance_model->get_atasan($username);
...
public function add_dataTambahan_post()
 ...
 public function datavaksinkeluarga_detail_post()
	{
		$_POST = $this->security->xss_clean($_POST);
        ...
        if ($this->form_validation->run() == FALSE){
        	...
        	if ($token_valid->status === FALSE) {
				...
				$get_date = date("Y-m-d");
	        		...
	        		$get_vaksinAll = $this->Attendance_model->vaksincheckkeluarga($token_valid->data->EMPLOYEE_NOPEK);
...
public function add_vaksin_post()
	{
		...
		$this->form_validation->set_rules('vaknama', 'Nama Vaksin', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('vakdate', 'Tanggal Vaksin', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('flag', 'flag', 'trim|required|max_length[5]');
        if ($this->form_validation->run() == FALSE){
            // form validation error
            $message = array('status' => false,
             				 'message' => $this->form_validation->error_array());
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
						...
						$datavaksin = [
							...
							"VaccineName" => $this->input->post('vaknama') ,
...
public function add_vaksin_kel_post()
	{
		...
		$this->form_validation->set_rules('vaknama', 'Nama Vaksin', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('vakdate', 'Tanggal Vaksin', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('nama', 'Nama Keluarga', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('flag', 'flag', 'trim|required|max_length[5]');
        if ($this->form_validation->run() == FALSE){
            // form validation error
            $message = array('status' => false,
...
public function add_keluhankel_post()
	...
	public function edit_vaksin_post()
	{
		...
		// Form Validation
		$this->form_validation->set_rules('username', 'Username', 'trim|required|max_length[1000]'); 
		$this->form_validation->set_rules('id', 'Id Vaksin', 'trim|max_length[1000]');
		$this->form_validation->set_rules('vakke', 'Vaksin Ke', 'trim|max_length[1000]');
		$this->form_validation->set_rules('vaknama', 'Vaksin Nama', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('vakdate', 'Vaksin Date', 'trim|required|max_length[1000]'); 
        if ($this->form_validation->run() == FALSE){
            // form validation error
            $message = array('status' => false,
             				 'message' => $this->form_validation->error_array());
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
					  ...
					  $vaksin_update = $this->Attendance_model->edit_vaksin( $this->input->post('id'),$this->input->post('vakke'),$this->input->post('vaknama'),$this->input->post('vakdate'), $get_fulldate);
								...
								$message = array('status' => true,
														 'message' => '<b style="font-size:13px;text-transform: uppercase;">Data Vaksin Berhasil di Update!</b> ');
										return $this->response($message, REST_Controller::HTTP_OK);
	...
	public function edit_vaksinkel_post()
	{
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
					  ...
					  $vaksin_update = $this->Attendance_model->edit_vaksin_kel( $this->input->post('id'),$this->input->post('vakke'),$this->input->post('nama'),$this->input->post('vaknama'),$this->input->post('vakdate'), $get_fulldate);
	...
	public function edit_isolasi_post()
	{
		...
		// Form Validation
		$this->form_validation->set_rules('username', 'Username', 'trim|required|max_length[1000]'); 
		$this->form_validation->set_rules('id', 'Id isolasi', 'trim|max_length[1000]');
		$this->form_validation->set_rules('nama', 'nama keluarga', 'trim|max_length[1000]');
		$this->form_validation->set_rules('keluhan', 'keluhan keluarga', 'trim|required|max_length[1000]');
        if ($this->form_validation->run() == FALSE){
            // form validation error
            $message = array('status' => false,
             				 'message' => $this->form_validation->error_array());
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
					  ...
					  $isolasi_update = $this->Attendance_model->edit_isolasi( $this->input->post('id'),$this->input->post('nama'),$this->input->post('keluhan'), $get_fulldate);
								...
								$message = array('status' => true,
														 'message' => '<b style="font-size:13px;text-transform: uppercase;">Data Isolasi Mandiri Keluarga/Kerabat Berhasil di Update!</b> ');
										return $this->response($message, REST_Controller::HTTP_OK);
	...
	public function edit_phone_post()
	{
		...
		$this->form_validation->set_rules('id', 'Id isolasi', 'trim|max_length[1000]');
		$this->form_validation->set_rules('nama', 'nama keluarga', 'trim|max_length[1000]');
		$this->form_validation->set_rules('phone', 'phone keluarga', 'trim|required|max_length[1000]');
        if ($this->form_validation->run() == FALSE){
            // form validation error
            $message = array('status' => false,
             				 'message' => $this->form_validation->error_array());
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
					  ...
					  $isolasi_update = $this->Attendance_model->edit_phone( $this->input->post('id'),$this->input->post('nama'),$this->input->post('phone'), $get_fulldate);
								...
								$message = array('status' => true,
														 'message' => '<b style="font-size:13px;text-transform: uppercase;">Data No. Telepon Keluarga Berhasil di Update!</b> ');
										return $this->response($message, REST_Controller::HTTP_OK);
	...
	public function edit_keluhan_post()
	{
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
					  ...
					  $isolasi_update = $this->Attendance_model->edit_keluhan( $this->input->post('id'),$this->input->post('nama'),$this->input->post('keluhan'), $get_fulldate);
...
public function edit_wr_post()
	{
		...
		$this->form_validation->set_rules('username', 'Username', 'trim|required|max_length[1000]'); 
		$this->form_validation->set_rules('id', 'Id isolasi', 'trim|max_length[1000]');
		$this->form_validation->set_rules('kegiatan', 'Kegiatan working report', 'trim|max_length[1000]');
		$this->form_validation->set_rules('hasil', 'Hasil working report', 'trim|max_length[1000]');
		$this->form_validation->set_rules('status', 'Status working report', 'trim|max_length[1000]');
        if ($this->form_validation->run() == FALSE){
            // form validation error
            $message = array('status' => false,
             				 'message' => $this->form_validation->error_array());
        	...
        	if ($token_valid->status === FALSE) {
        		...
        		if ($username != $token_valid->data->USERNAME) {
		            ...
		            return $this->response($message, REST_Controller::HTTP_OK);	
					  ...
					  $wr_update = $this->Attendance_model->edit_wr( $this->input->post('id'),$this->input->post('kegiatan'),$this->input->post('hasil'),$this->input->post('status'));
								...
								$message = array('status' => true,
														 'message' => '<b style="font-size:13px;text-transform: uppercase;">Data Working Report Berhasil di Update!</b> ');
										return $this->response($message, REST_Controller::HTTP_OK);
	...
	//public function  template_email_get()
	//{   
	//$dtsubject = $this->Attendance_model->getsubject()->result(); 
	//foreach ($dtsubject   as $row_subject)	
	//$dtbody = $this->Attendance_model->getbody()->result(); 
	//foreach ($dtbody   as $row_body)				
	//$dtcc = $this->Attendance_model->getcc()->result(); 
	//foreach ($dtcc   as $row_cc)				
	//$dtbcc = $this->Attendance_model->getbcc()->result(); 
	//foreach ($dtbcc   as $row_bcc)			
	//$dturl = $this->Attendance_model->geturlemail()->result(); 
	//foreach ($dturl   as $row_url)					
								
	//	$data_return[]  =  [
	        				//		"URL_EMAIL" =>  $row_url->FITUR_KEYVALUE,
	        				//		"SUBJECT_EMAIL" =>  $row_subject->FITUR_KEYVALUE,
	        				//		"BODY_EMAIL" =>  $row_body->FITUR_KEYVALUE,
	        				//		"CC_EMAIL" =>  $row_cc->FITUR_KEYVALUE,
	        				//		"BCC_EMAIL" =>  $row_bcc->FITUR_KEYVALUE
 	 								
	        					//] ;		

 								
								
			  //$message = array('dataAPIEMAIL' => $data_return );
				//	return $this->response($message, REST_Controller::HTTP_OK);	
	//}
		...
		public function sendemail_post()
	{
		...
		// Form Validation
		 $this->form_validation->set_rules('username', 'username', 'trim|required|max_length[100]');
		//$this->form_validation->set_rules('email', 'email dokter', 'trim|required|max_length[1000]'); 
		$this->form_validation->set_rules('zone', 'zona', 'trim|required|max_length[1000]'); 
		//$this->form_validation->set_rules('emailatasan', 'emailatasan', 'trim|required|max_length[1000]'); 
		$this->form_validation->set_rules('keluhan', 'keluhan', 'trim|required|max_length[1000]'); 
	...
	$get_BCC = $this->Attendance_model->getbcc()->result(); 
	foreach ($get_BCC   as $row_bcc)	
		...
		$url = $row_url->FITUR_KEYVALUE;
		$subject = '[PEPC – DIGITAL ATTENDANCE] Keluhan Kesehatan Pekerja Bapak/Ibu '.$token_valid->data->EMPLOYEE_NAMA.'';
		$char = array("\r", "\n", "\t","\\r", "\\n", "\\t"  );
						date_default_timezone_set('Asia/Jakarta');
						$get_date = date("Y-m-d");
						$get_time = date('H:i:s');
	   
	   $getAdditionalInfo= $this->Attendance_model->datatambahancheck($token_valid->data->EMPLOYEE_NOPEK);
			foreach ($getAdditionalInfo->result() as $row) {
						$PhoneNumber=$row->PhoneNumber;
								 
					}	
		
		$getKuesionerAbsen= $this->Attendance_model->kuischeck($token_valid->data->EMPLOYEE_NOPEK, $get_date);
			foreach ($getKuesionerAbsen->result() as $rowkuis) {
						$IsolasiMandiri=$rowkuis->IsolasiMandiri;
						$KeluhanPekerja=$rowkuis->KeluhanPekerja;
						$LokasiKerja=$rowkuis->LokasiKerja;
...
<tr>
<td colspan="3"   style="font-size:12px">Dengan Hormat,</td>
<td></td>
</tr>
<tr>
<td colspan="3"  style="font-size:12px">
Berikut diinformasikan pekerja di <b>'.$LokasiKerja.'</b>  yang mengalami keluhan kesehatan sebagai berikut :
</td> 
</tr>
<tr>
<td colspan="3">
...
<tr>
<td  style="font-size:12px">Nama Pekerja</td>
<td style="font-size:12px">:</td>
<td  style="font-size:12px">'.$token_valid->data->EMPLOYEE_NAMA.'  </td>
</tr> 
<tr>
<td style="font-size:12px">Nopek</td>
<td style="font-size:12px">:</td>
<td style="font-size:12px">'.$token_valid->data->EMPLOYEE_NOPEK.' </td>
</tr> 
<tr>
<td style="font-size:12px">Fungsi</td>
<td style="font-size:12px">:</td>
<td style="font-size:12px">'.$token_valid->data->COSTCENTERNAME.' </td>
</tr> 
<tr>
<td style="font-size:12px">Email</td>
<td style="font-size:12px">:</td>
<td style="font-size:12px">'.$token_valid->data->EMPLOYEE_EMAIL.' </td>
</tr> 
<tr>
<td style="font-size:12px">Nomor HP</td>
<td style="font-size:12px">:</td>
<td style="font-size:12px">'.$PhoneNumber.' </td>
</tr> 
<tr>
<td style="font-size:12px">Isolasi Mandiri</td>
<td style="font-size:12px">:</td>
<td style="font-size:12px">'.$IsolasiMandiri.' </td>
</tr> 
<tr>
<td style="font-size:12px">Keluhan Kesehatan</td>
<td style="font-size:12px">:</td>
<td style="font-size:12px">'.$this->input->post('keluhan').' </td>
</tr> 
<tr>
<td colspan="3">
<br>
...
<tr>
<td colspan="3" style="font-size:12px">
Diharapkan dari tim Health terkait untuk dapat melakukan monitoring dan tindak lanjut terhadap pekerja yang bersangkutan.<br><br>
Terima Kasih.<br><br>
<i>This is an automated generated e-mail message from PEPC Digital Attendance application, please do not reply to this email.</i>
</td> 
</tr>
</table>
</body>
</html>'; 
		$body =str_replace($char, "", $bodyText); 
		$cc = $token_valid->data->ATASAN_EMAIL; //$this->input->post('emailatasan');
		$bcc =$row_bcc->FITUR_KEYVALUE;
		$email = $this->input->post('email');
			...
			// kirim email
				...
				"subject" => $subject,
				"body" => $body,
				"mailto" => join('; ', $data_return),
				"cc" => $cc,
				"bcc" => $bcc
			   ];
			
			   $resultSendEmail = $this->Attendance_model->sendEmail($url, $paramemail); 
			    
			  
				 

					$message = array('dataSendEmail' => $resultSendEmail, 'requestJSON' => $paramemail,'URLSendEmail' => $url);
					return $this->response($message, REST_Controller::HTTP_OK);	
	...
	public function sendemailPersonel_post()
	{
		// XSS Filtering (https://www.codeigniter.com/user_guide/libraries/security.html)
		$_POST = $this->security->xss_clean($_POST);
		// Form Validation
		 $this->form_validation->set_rules('username', 'username', 'trim|required|max_length[100]');
		$this->form_validation->set_rules('zone', 'zona', 'trim|required|max_length[1000]'); 
		//$this->form_validation->set_rules('datadokter', 'datadokter', 'trim|required|max_length[1000]'); 
		$this->form_validation->set_rules('keluhan', 'keluhan', 'trim|required|max_length[1000]'); 
		//$this->form_validation->set_rules('ccemail', 'emailatasan', 'trim|required|max_length[1000]'); 
        	...
        	if ($token_valid->status === FALSE) {
				...
				$get_dataDok = $this->Attendance_model->doktorcheck($this->input->post('zone') );
					...
					foreach ($get_dataDok->result() as $row) { 
								 	   ...
								 	   $data_return[]=  '<tr><td>'.$row->NamaDokter.'</td><td>'.$row->Email.'</td><td>'.$row->NomorTelepon.'</td></tr>' ; 
					}
...
<tr>
<td colspan="3" style="font-size:12px">
Terima kasih atas laporan keluhan kesehatan yang telah disampaikan sebagai berikut :
</td> 
</tr>
<tr>
<td colspan="3">
...
<tr>
<td colspan="3" style="font-size:12px">
Dengan ini disampaikan bahwa Perusahaan telah menerima info bahwa Bapak/Ibu mengalami keluhan kesehatan.<br>
Disarankan kepada Bapak/Ibu untuk melakukan <i>Work From Home</i>, tetap patuhi protokol kesehatan 6M, lakukan vaksinasi, dan isolasi mandiri jika diperlukan.
<br><br>
Apabila Bapak/Ibu membutuhkan bantuan atau pertolongan lebih lanjut, silahkan menghubungi PIC Health berikut : 
</td> 
</tr>
...
<tr>
<td style="text-align:center;font-size:12px"><b>Nama Dokter</b></td>
<td style="text-align:center;font-size:12px"><b>Alamat Email</b></td>
<td style="text-align:center;font-size:12px"><b>Nomor HP</b></td>
</tr>
...
<i>This is an automated generated e-mail message from PEPC Digital Attendance application, please do not reply to this email.</i>
	...
	public function update_kondisi_post()
	{
		// XSS Filtering (https://www.codeigniter.com/user_guide/libraries/security.html)
		$_POST = $this->security->xss_clean($_POST);
		// Form Validation
		$this->form_validation->set_rules('status_isoman', 'status isoman', 'trim|required|max_length[10]'); 
		$this->form_validation->set_rules('status_kesehatan', 'status kesehatan', 'trim|required|max_length[10]'); 
		$this->form_validation->set_rules('nopek', 'nopek karyawan', 'trim|required|max_length[100]'); 
		$this->form_validation->set_rules('date', 'date absen', 'trim|required|max_length[100]');
		$this->form_validation->set_rules('note', 'keterangan kesehatan', 'trim|required|max_length[1000]');
		$this->form_validation->set_rules('note_lain', 'keterangan kesehatan lainnya', 'trim|required|max_length[1000]');
		 
		
		$status_isoman = $this->input->post('status_isoman');
		$status_kesehatan = $this->input->post('status_kesehatan');
		$nopek = $this->input->post('nopek');
		$date = $this->input->post('date');
		$note = $this->input->post('note');
		$note_lain = $this->input->post('note_lain');
		...
		{
			if($status_isoman=='Ya' && $status_kesehatan=='Ya' ){
				$statusFit=0;
				$noteFit=$note;
				$noteFitlain=$notelain;
			}else if($status_isoman=='Ya' && $status_kesehatan=='Tidak' ){
				$statusFit=0;
				$noteFit=$note;
				$noteFitlain=$notelain;
			}else if($status_isoman=='Tidak' && $status_kesehatan=='Ya' ){
				$statusFit=0;
				$noteFit=$note;
				$noteFitlain=$notelain;
			}else {
			   ...
			   $resultstatus = $this->Attendance_model->updateFit($nopek, $date, $statusFit, $noteFit, $noteFitlain); 
			    
			   			 	

				
								$message = array('status' => true,
														 'message' => '<b style="font-size:13px;text-transform: uppercase;">Berhasil update status fitunfit</b> ');
										return $this->response($message, REST_Controller::HTTP_OK);
